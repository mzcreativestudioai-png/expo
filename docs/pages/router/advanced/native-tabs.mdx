...
Install the icon set you want to use along with `@react-native-vector-icons/get-image`, which provides the native module that `getImageSourceSync` relies on:
...
`getImageSourceSync` is synchronous, so compute the image sources once at module scope rather than on each render. Combine `src` with `sf` to use SF Symbols on iOS and the vector icon on Android. On iOS, `sf` takes precedence over `src`; on Android, the icon falls back to `src`.
